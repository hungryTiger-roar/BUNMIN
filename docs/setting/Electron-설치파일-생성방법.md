...
VLM Base(`Qwen/Qwen3-VL-8B-Instruct`)는 17GB. 동봉할지에 따라 두 가지 빌드:
...
```bash
cp -r models/qwen3-vl-8b-instruct setup/win-unpacked/resources/backend/models/
```

이후 단계 3 ISCC 실행. 압축 시간이 ~30~40분으로 늘어남.

> dev 머신에 `models/qwen3-vl-8b-instruct/`가 있어야 가능. 없으면 `npm run setup`으로 먼저 받기.
...
1. `%LOCALAPPDATA%/Aunion AI/models/qwen3-vl-8b-instruct/` (사용자 데이터)
2. `<install>/resources/backend/models/qwen3-vl-8b-instruct/` (동봉본)
3. 없으면 HF repo_id `Qwen/Qwen3-VL-8B-Instruct`로 다운로드

다운로드된 모델은 HF 캐시(`%LOCALAPPDATA%/Aunion AI/cache/huggingface/`)에 들어감.

### `phontab: No such file or directory` (TTS 에러)

`piper`의 `espeak-ng-data`가 PyInstaller 번들에 안 들어간 경우. [aunion.spec](../../backend/aunion.spec)에 `collect_data_files('piper')`가 있는지 확인. 없으면 추가 후 PyInstaller 재빌드.
...
이미 적용되어 있으면 재빌드만 하면 해결. 임시 우회: 설치 위치의 `resources\backend\models\qwen3-vl-8b-instruct` 빈 폴더 삭제 → 앱 재시작 → 자동으로 HF에서 다운로드 시도.
