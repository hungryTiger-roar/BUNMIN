...
1. **페이지당 처리 시간이 가변적**
   ...
   - VLM 번역(Qwen3-VL-4B-Instruct, 4bit): 텍스트 길이 / 토큰 수에 따라 15 ~ 60초 편차
...
_BASELINE_SECONDS_PER_PAGE = {
    ...
    "translate": 50.0, # Qwen3-VL-4B 4bit GPU 한 장 번역 추정치 — 첫 실행 시 fallback
}
...
_BASELINE_SECONDS_PER_PAGE = {
    ...
    "translate": 50.0, # Qwen3-VL-4B 4bit GPU 한 장 번역 추정치
}
