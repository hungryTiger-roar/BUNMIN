    ...
    <div className={wrapperClass}>
      ...
      {onClose && (
        <div className={`px-4 py-3 flex items-center justify-between border-b ${c.headerBorder} flex-shrink-0`}>
          <h3 className={`text-lg wide:text-base font-medium ${c.textPrimary}`}>{t.title}</h3>
          <button
            onClick={onClose}
            className={`w-8 h-8 wide:w-7 wide:h-7 flex items-center justify-center rounded-full text-lg wide:text-base ${c.textMuted} ${c.closeHover} transition-colors`}
            aria-label={t.close}
          >
            ✕
          </button>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-1 min-h-0">
        {participants.lecturer && (
          <div className={`flex items-center gap-3 px-3 py-3 wide:py-2 rounded-lg ${c.lecturerBg}`}>
            <div className={`w-10 h-10 wide:w-9 wide:h-9 rounded-full ${c.iconBg} flex items-center justify-center ${c.iconColor}`}>
            ...
            <div className="flex-1 min-w-0">
              <div className={`text-lg wide:text-sm font-medium truncate ${c.textPrimary}`}>
                {participants.lecturer.name || t.professorFallback}
              </div>
            </div>
            {participants.lecturer.connected && (
              <span className="w-3 h-3 wide:w-2 wide:h-2 rounded-full bg-green-500" />
            )}
          </div>
        )}
        {studentList.map((s) => (
          <div
            key={s.id}
            className={`flex items-center gap-3 px-3 py-3 wide:py-2 rounded-lg ${c.rowHover}`}
          >
            <div className={`w-10 h-10 wide:w-9 wide:h-9 rounded-full ${c.iconBg} flex items-center justify-center text-lg wide:text-sm font-semibold ${c.iconColor} uppercase`}>
              {(s.name || '?').charAt(0)}
            </div>
            <div className="flex-1 min-w-0">
              <div className={`text-lg wide:text-sm font-medium truncate ${c.textPrimary}`}>
        ...
        {showFallback && (
          <div className={`text-lg wide:text-sm text-center py-4 ${c.textMuted}`}>
            {t.loadingNames(fallbackStudentCount)}
          </div>
        )}
        {!participants.lecturer && studentList.length === 0 && !showFallback && (
          <div className={`text-lg wide:text-sm text-center py-8 ${c.textMuted}`}>
            {t.noParticipants}
          </div>
        )}
      </div>
