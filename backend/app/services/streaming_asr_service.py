...
# 의문문 보강 (2026-05): ~인가? / ~던가? / ~나요? / ~을래? / ~을지? / ~다고? / ~다면서?
# 같은 의문 종결어미 폭넓게 포함. 강의 중 강사가 자주 쓰는 구어체 의문문 모두 finalize 됨.
_SENTENCE_TERMINATOR = re.compile(
    r"[가-힣]*?"                              # 종결어미 앞 음절
    r"("
    # === 4글자 (확인 의문 존댓말) ===
    r"다면서요|라면서요|"                     # ~다면서요? / ~라면서요?
    # === 격식체 (formal) — 4~3글자 ===
    r"습니다|습니까|입니다|입니까|"
    r"십시오|십시다|ㅂ니다|ㅂ니까|"
    r"답니다|답니까|"                          # 인용 (대화체)
    ...
    r"잖니요|구나요|"
    # === 존댓말 의문 — 3글자 (의문 보강) ===
    r"인가요|ㄴ가요|은가요|는가요|던가요|"   # ~인가요? / ~던가요? 등
    r"을래요|ㄹ래요|"                          # ~을래요? / ~ㄹ래요? (의향 의문)
    r"을지요|ㄹ지요|"                          # ~을지요? / ~ㄹ지요? (미정 의문)
    r"다고요|라고요|자고요|냐고요|"            # ~다고요? / ~라고요? (인용 의문 존댓말)
    # === 반말 확인 의문 — 3글자 ===
    r"다면서|라면서|"                          # ~다면서? / ~라면서?
    # === 존댓말 -요 종결 — 2글자 ===
    ...
    r"ㄹ까|ㄹ게|을까|을게|"
    # === 반말 의문 — 2글자 (의문 보강) ===
    r"인가|ㄴ가|은가|는가|던가|"               # ~인가? / ~던가? 등
    r"을래|ㄹ래|"                              # ~을래? / ~ㄹ래? (의향 의문)
    r"을지|ㄹ지|"                              # ~을지? / ~ㄹ지? (미정 의문)
    r"다고|라고|자고|냐고|"                    # ~다고? / ~라고? (인용 의문 / 반문)
    r"으냐|느냐|"                              # ~으냐? / ~느냐? (격식 의문, 다소 옛스러움)
    r"다며|라며|"                              # ~다며? / ~라며? (확인 의문 단축형)
    # === 반말 + -요 (1글자 + 요) ===
    r"죠|"
    # === 반말 1글자 종결 (구두점 필수) ===
    r"네|군|데|대|"
    # === 단음절 종결 (구두점이 강제라 false positive 적음) ===
    r"다|요|까|야|자|어|아|지|니|냐|래|"      # "래" 추가 (~할래?, ~갈래? 의향)
    # === 의문/감탄 ===
    r"나"                # "어쩌나.", "어디나." 같은 케이스
    r")"
...
class ASRStreamingService:
    ...
    async def _force_finalize(self) -> list[str]:
        ...
        # 꼬리 환각 trim — _extract_finalized 와 동일하게 적용. max_buffer 강제 finalize
        # 시 꼬리에 붙은 "감사합니다" / "시청해주셔서 감사합니다" 같은 환각 정형구 제거.
        # 작별 동사 (만나/봬/봐/뵐) 결합 시만 매칭하므로 정상 강의 예고는 보존됨.
        cleaned: list[str] = []
        for s in sentences:
            if not s:
                continue
            trimmed = _TRAILING_HALLUCINATION.sub("", s).strip()
            if trimmed:
